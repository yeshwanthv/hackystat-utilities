...
public class TestUriCache {
  ...
  public void testHierarchicalKeyCacheRemoval() {
    // Create a cache. 
    UriCache cache = new UriCache("HierarchicalKeyCache", testSubDir, 1D, 1L);
    cache.clear();
  ...
  public void testDisposeCache() {
    ...
    UriCache cache = new UriCache(cacheName, testSubDir);
    cache.put(key, value);
    assertEquals("Checking simple get", value, cache.get(key));
    // now dispose and try again.
    UriCache.dispose(cacheName);
    cache = new UriCache(cacheName, testSubDir);
    cache.put(key, value);
  ...
  public void testDiskCache() {
    ...
    Long capacity = 100L;
    UriCache cache = new UriCache("TestDiskCache", testSubDir, maxLifeDays, capacity);
  ...
  public void testElementExpiration() throws Exception {
    ...
    Long capacity = 100L;
    UriCache cache = new UriCache("TestExpiration", testSubDir, maxLifeDays, capacity);
